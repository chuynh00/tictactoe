...
class player1UIClass():
    ...
    def canvasSetup(self):
        ...
        self.master.configure(background='light blue')  # set the background color of the window
    ...
    def buttonClicked(self, button):
        ...
        self.player1Move = ['X', self.pos]
        self.player1Board.updateGameBoard(self.player1Move)
        self.player1Board.printBoard()

        # Send my move to player2
        self.player1Socket.send(bytes(str(self.player1Move), encoding='ascii'))

        # Check to see if Player1 is a winner
        self.win = self.player1Board.isWinner("X")

        # If player1 is the winner, announce they are the winner in a pop up message
        # then ask if they want to play again. If yes then clear board from BoardClass and UI
        # then send over to player2 that we requested to play again
        if self.win == True:
            if self.winner_announce1() == True:
                self.player1Board.resetGameBoard()

                for button in self.buttonList:
                    button['text'] = ' '

                self.player1Socket.send(bytes("PLAY AGAIN", encoding='ascii'))
                return True
            else:
                self.player1Socket.send(bytes("Fun Times", encoding='ascii'))
                self.player1Board.printStats()
                self.createFinalResultsPopUp()
                self.master.quit()

        # Check to see if there is a tie after player1 puts move down, announce Tie in a pop up message
        # then ask if they want to play again. If yes then clear board from BoardClass and UI
        # then send over to player2 that we requested to play again
        self.tie = self.player1Board.boardIsFull()
        if self.tie == True:
            if self.tieAnnounce() == True:
                self.player1Board.resetGameBoard()
        ...
        # Change current turn to player2 before receiving their move
        self.player1Board.changeCurrentPlayer()
        self.currentTurnLabel.config(text="Current Turn:" + self.player1Board.__currentTurn__)
        self.master.update()
        print("Current Turn:", self.player1Board.__currentTurn__)


        # Listen for Player2's move
        ...
        # Update Board Class
        self.player1Board.updateGameBoard(self.splitstring)
        self.player1Board.printBoard()

        # Update Player1's UI board according to Player2's sent move
        ...
        # Check to see if Player1 is the loser
        self.win = self.player1Board.isWinner("X")
        ...
        # Check to see if there is a Tie after player2 puts move down, announce Tie in a pop up message
        # then ask if they want to play again. If yes then clear board from BoardClass and UI
        # then send over to player2 that we requested to play again
        self.tie = self.player1Board.boardIsFull()
        if self.tie == True:
    ...
    def createStartButton(self):
        self.startButton = Button(self.master, text="Start Server", command=self.startServer)
        self.startButton.grid(row=4, column=5)
    ...
    def startServer(self):
        ...
        try:
            # Create socket object for Player1
            self.player1Socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            self.myconnect = (self.address.get(), self.port.get())

            # Attempt to connect to the server of Player2
            self.player1Socket.connect(self.myconnect)

            # Call function to pop up message box to indicate connection was successful
            self.connectSuccess()
            print("Successfully Connected!")

        except (ConnectionError, ConnectionRefusedError, ConnectionResetError, ConnectionAbortedError, OSError):
            # Call function to pop up error message indicating connection was unsuccessful
            self.connect_again = self.connectFail()

            # if user click yes, nothing happens but if user clicks no then destroy UI and program
            if self.connect_again == 'yes':
                pass
            else:
                self.master.destroy()
    ...
    def send_username(self):
        # Player 1 sends username first
        self.player1Socket.send(bytes(self.myUsername.get(), encoding='ascii'))

        # Then receives Player2's username
        self.otherPlayerUserName.set(self.player1Socket.recv(1024).decode('ascii'))
        print("Player 2's username is:", self.otherPlayerUserName.get())

        self.gameloop()


    # define a method to start the gameloop
    def gameloop(self):
        self.player1Board = game.BoardClass((self.myUsername.get(), self.otherPlayerUserName.get()))

        self.createCurrentTurnLabel()
        self.currentTurnLabel['text'] = "Current Turn: " + self.player1Board.__currentTurn__
        print("Current Turn:", self.player1Board.__currentTurn__)
    ...
    def createCurrentTurnLabel(self):
        self.currentTurnLabel = tk.Label(self.master, text="Current turn: " + self.player1Board.__currentTurn__)
        self.currentTurnLabel.grid(row=6, column=5)


    # define a method to display final results
    def createFinalResultsPopUp(self):
        self.finalResults = showinfo(title="Player 1 Stats", message="Final Results: \n" +
                                                            "Players usernames are: " + str(self.player1Board.__playerUserNames__) + '\n' +
                                                            "Last person to make a move :" + str(self.player1Board.__currentTurn__) + '\n' +
                                                            "Number of games played :" + str(self.player1Board.__numGames__) + '\n' +
                                                            "Number of wins: " + str(self.player1Board.__numWins__) + '\n' +
                                                            "Number of losses: " + str(self.player1Board.__numLoss__) + '\n' +
                                                            "Number of ties: " + str(self.player1Board.__numTies__))
    ...
    def winner_announce1(self):
        self.playagain = askyesno(title="Winner Detected", message="Player 1 is the winner. Player 2 is the loser. Do you want to play again?")
        return self.playagain

    def loser_annouce1(self):
        self.playagain = askyesno(title="Winner Detected", message="Player 1 is the loser. Player 2 is the winner Do you want to play again?")
        return self.playagain
